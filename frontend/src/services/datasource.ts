...
class DatasourceService {
  ...
  async getDatasourceUsage(): Promise<DatasourceUsage[]> {
    const response = await axios.get<DatasourceUsage[]>(`${API_BASE_URL}/usage`);
    return response.data;
  }
  ...
  // 7. 测试数据源连接
  async testConnection(id: number): Promise<ApiResponse<void>> {
    const response = await axios.post<ApiResponse<void>>(`${API_BASE_URL}/${id}/test`);
    return response.data;
  }
