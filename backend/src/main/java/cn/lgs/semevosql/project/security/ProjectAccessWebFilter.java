...
@ConditionalOnProperty(name = "semevosql.security.enabled", havingValue = "true")
public class ProjectAccessWebFilter implements WebFilter {

	private static final Pattern PROJECT = Pattern.compile("^/api/semevosql/projects/(\\d+)(?:/.*)?$");

	private static final Pattern PROJECT_CONVERSATION_CREATE = Pattern
		.compile("^/api/semevosql/projects/(\\d+)/conversations$");

	private static final Pattern PROJECT_CONVERSATION_SEND = Pattern
		.compile("^/api/semevosql/projects/(\\d+)/conversations/[^/]+/messages$");

	private static final Pattern PROJECT_CONVERSATION_RUN_ACTION = Pattern
		.compile("^/api/semevosql/projects/(\\d+)/conversations/[^/]+/runs/[^/]+/(?:human-review|sync)$");

	private static final Pattern QUERY_BINDING_CORRECTION = Pattern.compile(
			"^/api/semevosql/projects/(\\d+)/conversations/[^/]+/runs/[^/]+/corrections/binding$");

	private static final Pattern OPERATIONS_PROJECT = Pattern
		.compile("^/api/semevosql/operations/projects/(\\d+)(?:/.*)?$");

	private static final Pattern RUN = Pattern.compile("^/api/semevosql/runs/([^/]+)(?:/.*)?$");

	private static final Pattern GAP = Pattern.compile("^/api/semevosql/gaps/(\\d+)(?:/.*)?$");

	private static final Pattern EVOLUTION_CANDIDATE = Pattern
		.compile("^/api/semevosql/semantic-evolution/candidates/([^/]+)(?:/.*)?$");

	private static final Pattern REPLAY_RUN = Pattern
		.compile("^/api/semevosql/semantic-evolution/replay-runs/([^/]+)(?:/.*)?$");

	private static final Pattern OPTIMIZATION_CANDIDATE = Pattern
		.compile("^/api/semevosql/runtime-optimization/candidates/([^/]+)(?:/.*)?$");

	private static final Pattern JOB = Pattern.compile("^/api/semevosql/operations/jobs/([^/]+)(?:/.*)?$");

	private static final Pattern RELEASE = Pattern.compile("^/api/semevosql/operations/releases/([^/]+)(?:/.*)?$");

	private static final Pattern EPISODE = Pattern.compile("^/api/semevosql/operations/episodes/([^/]+)(?:/.*)?$");

	private static final Pattern ATTEMPT = Pattern.compile("^/api/semevosql/operations/attempts/([^/]+)(?:/.*)?$");

	private static final Pattern TRAJECTORY_PATTERN = Pattern
		.compile("^/api/semevosql/trajectory/patterns/([^/]+)(?:/.*)?$");
	...
	public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
		...
		if (!path.startsWith("/api/semevosql/")) {
	...
	private boolean viewerRuntimeMutation(String path, HttpMethod method) {
		...
		return PROJECT_CONVERSATION_CREATE.matcher(path).matches()
				...
				|| path.matches("^/api/semevosql/operations/episodes/[^/]+/feedback$")
				|| path.matches("^/api/semevosql/runs/[^/]+/(?:cancel|resume)$")
				|| path.matches("^/api/semevosql/runs/[^/]+/clarification/[^/]+/answer$");
	}
