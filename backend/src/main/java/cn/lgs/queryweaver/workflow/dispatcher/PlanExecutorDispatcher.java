		...
		if (validationPassed) {
			...
			if (repairCount > MAX_REPAIR_ATTEMPTS) {
				String validationError = StateUtil.getStringValue(state, PLAN_VALIDATION_ERROR,
						"Plan validation failed after repair attempts");
				log.error("Plan repair attempts exceeded the limit of {}. Failing execution: {}", MAX_REPAIR_ATTEMPTS,
						validationError);
				throw new IllegalStateException(
						"Plan repair attempts exceeded the limit of " + MAX_REPAIR_ATTEMPTS + ": " + validationError);
			}
