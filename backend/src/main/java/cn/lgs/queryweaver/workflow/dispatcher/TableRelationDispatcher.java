...
public class TableRelationDispatcher implements EdgeAction {
	...
	public String apply(OverAllState state) throws Exception {
		...
		Optional<Object> tableRelationOutput = state.value(TABLE_RELATION_OUTPUT);
		if (tableRelationOutput.isPresent()) {
			return state.value(ADVANCED_EXECUTION_FALLBACK, false) ? PLANNER_NODE : SEMANTIC_PLAN_NODE;
		}
