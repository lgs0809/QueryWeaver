...
import cn.lgs.queryweaver.util.JsonParseUtil;
...
public class PythonExecuteNode implements NodeAction {
	...
	private Map<String, Object> replayPythonExecution(OverAllState state, PythonExecutionEffect effect) {
		Flux<ChatResponse> displayFlux = Flux.create(emitter -> {
			emitter.next(ChatResponseUtil.createResponse("恢复已持久化的Python执行结果，无需重复运行代码。"));
			emitter.next(ChatResponseUtil.createPureResponse(TextType.JSON.getStartSign()));
			emitter.next(ChatResponseUtil.createResponse(effect.output()));
			emitter.next(ChatResponseUtil.createPureResponse(TextType.JSON.getEndSign()));
			emitter.complete();
		});
		Flux<GraphResponse<StreamingOutput>> generator = FluxUtil.createStreamingGeneratorWithMessages(this.getClass(),
				state, ignored -> effectState(effect), displayFlux);
		return Map.of(PYTHON_EXECUTE_NODE_OUTPUT, generator);
	}

	private Map<String, Object> effectState(PythonExecutionEffect effect) {
		Map<String, Object> result = new java.util.HashMap<>();
		result.put(PYTHON_EXECUTE_NODE_OUTPUT, effect.output());
		result.put(PYTHON_IS_SUCCESS, effect.success());
		if (effect.fallback()) {
			result.put(PYTHON_FALLBACK_MODE, true);
		}
		return result;
	}

	private PythonExecutionEffect readEffect(String payload) {
		try {
			return objectMapper.readValue(payload, PythonExecutionEffect.class);
		}
		catch (Exception ex) {
			throw new IllegalStateException("Unable to restore persisted Python execution result", ex);
		}
	}

	private String writeEffect(PythonExecutionEffect effect) {
		try {
			return objectMapper.writeValueAsString(effect);
		}
		catch (Exception ex) {
			throw new IllegalStateException("Unable to persist Python execution result", ex);
		}
	}

	private record PythonExecutionEffect(String output, boolean success, boolean fallback) {
	}
