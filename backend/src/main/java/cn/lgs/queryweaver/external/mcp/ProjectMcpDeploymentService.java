...
public class ProjectMcpDeploymentService {
    ...
    public DeploymentCredential deploy(Long projectId, OperatorContext operator) {
        ...
        ProjectRuntimeContext runtime = runtimeGate.requireReadyByProject(projectId);
        ...
        if (existing == null) {
            ...
            deployment = new ProjectMcpDeployment(deploymentId, projectId, runtime.projectVersionId(),
                    runtime.catalogHash(), principalId, ProjectMcpDeployment.Status.RUNNING, properties.endpoint(),
                    operator.operator(), null, null, null, null);
            repository.insertDeployment(deployment);
        }
        else {
            repository.updateDeployment(existing.deploymentId(), runtime.projectVersionId(), runtime.catalogHash(),
                    ProjectMcpDeployment.Status.RUNNING);
    ...
    public ProjectMcpDeployment enable(Long projectId, OperatorContext operator) {
        ...
        ProjectRuntimeContext runtime = runtimeGate.requireReadyByProject(projectId);
        repository.updateDeployment(deployment.deploymentId(), runtime.projectVersionId(), runtime.catalogHash(),
                ProjectMcpDeployment.Status.RUNNING);
    ...
    public TestResult test(Long projectId, OperatorContext operator) {
        ...
        boolean bindingCurrent = deployment.projectVersionId().equals(runtime.projectVersionId())
                && deployment.catalogHash().equals(runtime.catalogHash());
        return new TestResult(running && bindingCurrent, running, bindingCurrent, deployment.endpoint());
    }
    ...
    private DeploymentCredential credentialResponse(ProjectMcpDeployment deployment, String token) {
        return new DeploymentCredential(deployment, token,
                new ConnectionConfig(deployment.endpoint(), "Authorization", "Bearer " + token,
                        "Remote Streamable HTTP BYO-Agent mode; use the bundled QueryWeaver MCP skill with search_semantics, get_semantic_context, validate_query_plan, execute_query_plan and get_query_result. Caller-side Agent tokens pay for reasoning; QueryWeaver's online MCP data plane does not call its Chat LLM."));
    }
