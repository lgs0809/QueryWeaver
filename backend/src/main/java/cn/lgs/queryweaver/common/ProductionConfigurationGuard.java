...
import cn.lgs.queryweaver.enums.CodePoolExecutorEnum;
...
public class ProductionConfigurationGuard implements ApplicationRunner {
	...
	public void run(ApplicationArguments args) {
		if (operatorProperties.isDevelopmentMode()) {
			throw invalid("queryweaver.operator.development-mode must be false");
		}
		CodePoolExecutorEnum executor = codeExecutorProperties.getCodePoolExecutor();
		String runtimeRole = environment.getProperty("queryweaver.runtime-role", "application").trim();
		...
		if (!Boolean.parseBoolean(required("queryweaver.security.enabled"))) {
			throw invalid("queryweaver.security.enabled must be true");
		}
		boolean projectMcpEnabled = Boolean.parseBoolean(environment.getProperty("queryweaver.mcp.enabled", "false"));
		...
		if (projectMcpEnabled != springMcpEnabled) {
			throw invalid("QueryWeaver and Spring AI MCP server enablement must match");
		}
		if (projectMcpEnabled) {
			...
			if (!"/mcp".equals(environment.getProperty("queryweaver.mcp.external.endpoint-path", "/mcp"))) {
		...
		required("queryweaver.secrets.encryption-key");
	...
	private IllegalStateException invalid(String message) {
		return new IllegalStateException("Unsafe QueryWeaver production configuration: " + message);
	}
