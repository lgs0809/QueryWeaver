...
public class ProjectSemanticAliasProposalService {
	...
	public ProjectSemanticAliasProposalService(JdbcTemplate jdbc, SemanticProjectRepository projectRepository,
			SemanticBindingTargetValidator targetValidator) {
		this.jdbc = jdbc;
	...
	public ProposalResult propose(Long projectId, Long sourceVersionId, String rawPhrase, String assetType,
		...
		if (existing != null) {
			...
			return new ProposalResult(existing.id(), sourceVersionId, normalizedPhrase, "CANDIDATE");
		}
		...
		jdbc.update("""
				...
				VALUES (?, ?, ?, ?, 'PROJECT_ALIAS_PROPOSAL', 'PROJECT_ALIAS', ?, 'CANDIDATE', 1.0, 'MEDIUM',
		...
		return new ProposalResult(persisted.id(), sourceVersionId, normalizedPhrase, "CANDIDATE");
	}
	...
	private PendingProposal findPending(Long sourceVersionId, String catalogHash, String normalizedPhrase) {
		return jdbc
			.query("""
					...
					""",
					(rs, rowNum) -> new PendingProposal(rs.getString("id"), rs.getString("target_asset_type"),
							rs.getString("target_asset_key")),
	...
	private record PendingProposal(String id, String targetAssetType, String targetAssetKey) {
	}

	public record ProposalResult(String candidateId, Long sourceVersionId, String normalizedPhrase, String status) {
	}
