...
import cn.lgs.queryweaver.project.application.ProjectRuntimeProfileService;
import cn.lgs.queryweaver.project.domain.ProjectRuntimeContext;
import cn.lgs.queryweaver.run.ExecutionSnapshotService;
import cn.lgs.queryweaver.run.ExecutionSnapshotService.ExecutionSnapshotMismatchException;
...
import cn.lgs.queryweaver.run.RunInProgressException;
...
import cn.lgs.queryweaver.service.graph.Context.MultiTurnContextManager;
import cn.lgs.queryweaver.service.graph.Context.StreamContext;
...
public class GraphServiceImpl implements GraphService {
	...
	public GraphServiceImpl(StateGraph stateGraph, @Qualifier("queryWeaverInteractiveExecutor") Executor executor,
		...
		try {
			...
			if (!StringUtils.hasText(episodeId)) {
				var binding = productionService.createFirstAttemptAndBind(run.runId(), run.threadId(),
						episodeRequest(run.requestId(), agentId, naturalQuery, projectContext));
				episodeId = binding.episodeId();
				attemptId = binding.attemptId();
				run = binding.run();
			}
			else if (runService.get(run.runId()).status() == RunStatus.QUEUED) {
				var binding = productionService.createNextAttemptAndBind(run.runId(), episodeId, run.threadId());
				attemptId = binding.attemptId();
				run = binding.run();
			}
	...
	private void prepareProjectRuntimeIdentity(GraphRequest request) {
		if (request == null || request.getProjectId() == null) {
			throw new IllegalArgumentException("projectId is required for QueryWeaver graph execution");
		}
	...
	private void handleNewProcess(GraphRequest graphRequest) {
		...
		if (!StringUtils.hasText(episodeId)) {
			var binding = productionService.createFirstAttemptAndBind(run.runId(), threadId,
					episodeRequest(run.requestId(), agentId, query, projectContext));
			episodeId = binding.episodeId();
			attemptId = binding.attemptId();
			run = binding.run();
		}
		else if (run.status() == RunStatus.QUEUED) {
			var binding = productionService.createNextAttemptAndBind(run.runId(), episodeId, threadId);
			attemptId = binding.attemptId();
			run = binding.run();
		}
		...
		try {
			...
			if (StringUtils.hasText(context.getEpisodeId())) {
				productionService.completeEpisode(context.getEpisodeId(),
						new QueryWeaverProductionService.CompletionRequest("SUCCEEDED", null, null,
			...
			if (terminal.status() == RunStatus.SUCCEEDED) {
				...
				if (StringUtils.hasText(context.getEpisodeId())) {
					try {
						productionService.completeEpisode(context.getEpisodeId(),
								new QueryWeaverProductionService.CompletionRequest("FAILED", errorCode, null,
										elapsedMillis(context)));
					}
	...
	private boolean finishCancellationIfRequested(StreamContext context) {
		...
		if (StringUtils.hasText(context.getEpisodeId())) {
			try {
				productionService.completeEpisode(context.getEpisodeId(),
						new QueryWeaverProductionService.CompletionRequest("CANCELLED", null, null,
								elapsedMillis(context)));
			}
			catch (RuntimeException episodeError) {
	...
	private void completeEpisodeBestEffort(String episodeId, String status, String errorCode, long elapsedMillis) {
		...
		try {
			productionService.completeEpisode(episodeId,
					new QueryWeaverProductionService.CompletionRequest(status, errorCode, null, elapsedMillis));
		}
		catch (RuntimeException episodeError) {
	...
	private QueryWeaverProductionService.EpisodeRequest episodeRequest(String requestId, String agentId, String query,
			ProjectRuntimeContext context) {
		return new QueryWeaverProductionService.EpisodeRequest(requestId, agentId, context.projectId(),
				context.projectVersionId(), null, context.catalogHash(), query, query, null, "v1");
	}
	...
	static String streamErrorMessage(Throwable error) {
		if (isTimeoutFailure(error)) {
			return "查询执行超时，请稍后重试；如果问题较复杂，可缩小查询范围后重试。";
		}
		String message = error == null ? null : error.getMessage();
		if (isUpstreamModelFailure(error, message)) {
			return "模型服务暂时不可用，请稍后重试。";
		}
		return StringUtils.hasText(message) ? message : "查询执行失败，请稍后重试。";
	}
