 ...
 *
 * <p>The Spring AI 1.1.0 OpenAI embedding request DTO is a generic record. In the packaged QueryWeaver runtime we observed
 * that request serializing to an empty JSON object even though the same client behaved correctly in an isolated unit
 * test. QueryWeaver only needs the stable OpenAI-compatible contract ({@code input}, {@code model}, optional
