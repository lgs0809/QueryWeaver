 ...
 */
package cn.lgs.queryweaver.project.infrastructure;

import cn.lgs.queryweaver.project.domain.ProjectDatasourceBinding;
...
@Mapper
public interface QueryWeaverProjectMapper {
	...
	@Insert("""
			...
			(project_id, version_no, version_number, status, parent_version_id, creation_mode, initialization_model_id,
			 analysis_status, analysis_error, source, evidence, catalog_hash, release_report, create_time, validated_time,
			 published_time, revision)
			VALUES
			(#{projectId}, #{versionNo}, #{versionNumber}, #{status}, #{parentVersionId}, #{creationMode},
			 #{initializationModelId}, #{analysisStatus}, #{analysisError}, #{source}, #{evidence}, #{catalogHash},
			 #{releaseReport}, #{createTime}, #{validatedTime}, #{publishedTime}, #{revision})
			""")
	@Options(useGeneratedKeys = true, keyProperty = "id", keyColumn = "id")
	int insertVersion(SemanticProjectVersion version);

	@Update("""
			UPDATE qw_project_version SET status = #{status}, initialization_model_id = #{initializationModelId},
			analysis_status = #{analysisStatus}, analysis_error = #{analysisError}, source = #{source}, evidence = #{evidence},
			catalog_hash = #{catalogHash}, release_report = #{releaseReport}, validated_time = #{validatedTime},
			published_time = #{publishedTime}, revision = revision + 1
			WHERE id = #{id} AND revision = #{revision}
			""")
	int updateVersion(SemanticProjectVersion version);

	@Select("""
			SELECT id, project_id, version_no, version_number, status, parent_version_id,
			creation_mode, initialization_model_id, analysis_status, analysis_error, source, evidence, catalog_hash,
			release_report, create_time, validated_time, published_time, revision
