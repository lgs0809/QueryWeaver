 ...
 */
package cn.lgs.queryweaver.worker;

import cn.lgs.queryweaver.properties.CodeExecutorProperties;
import cn.lgs.queryweaver.service.code.CodePoolExecutorService;
import cn.lgs.queryweaver.service.code.CodePoolExecutorService.TaskRequest;
import cn.lgs.queryweaver.service.code.CodePoolExecutorService.TaskResponse;
 ...
 * public QueryWeaver controllers.
 */
public final class CodeExecutionWorkerApplication implements AutoCloseable {
	...
	public static void run(String[] args) {
		...
		Runtime.getRuntime().addShutdownHook(new Thread(application::close, "queryweaver-execution-worker-shutdown"));
	}
	...
	public synchronized int start(int port) {
		...
		try {
			...
			httpExecutor = new ThreadPoolExecutor(threads, threads, 0L, TimeUnit.MILLISECONDS,
					new ArrayBlockingQueue<>(Math.max(1, properties.getThreadQueueSize())), runnable -> {
						Thread thread = new Thread(runnable,
								"queryweaver-execution-http-" + THREAD_SEQUENCE.incrementAndGet());
			...
			log.info("QueryWeaver execution worker listening on port {}", actualPort);
			return actualPort;
		}
	...
	static CodeExecutorProperties loadProperties(Map<String, String> environment) {
		...
		properties.setInternalToken(environment.get("QUERYWEAVER_EXECUTION_INTERNAL_TOKEN"));
		properties.setMaxCodeBytes(
				integer(environment, "QUERYWEAVER_EXECUTION_MAX_CODE_BYTES", properties.getMaxCodeBytes()));
		properties.setMaxInputBytes(
				integer(environment, "QUERYWEAVER_EXECUTION_MAX_INPUT_BYTES", properties.getMaxInputBytes()));
		properties.setMaxRequirementBytes(integer(environment, "QUERYWEAVER_EXECUTION_MAX_REQUIREMENT_BYTES",
				properties.getMaxRequirementBytes()));
		properties.setPidsLimit(longValue(environment, "QUERYWEAVER_EXECUTION_PIDS_LIMIT", properties.getPidsLimit()));
		properties
			.setTmpfsSizeMb(integer(environment, "QUERYWEAVER_EXECUTION_TMPFS_SIZE_MB", properties.getTmpfsSizeMb()));
		properties
			.setLimitMemory(longValue(environment, "QUERYWEAVER_EXECUTION_MEMORY_MB", properties.getLimitMemory()));
		properties.setCpuCore(longValue(environment, "QUERYWEAVER_EXECUTION_CPU_CORES", properties.getCpuCore()));
		properties.setRunAsUser(value(environment, "QUERYWEAVER_EXECUTION_RUN_AS_USER", properties.getRunAsUser()));
		properties
			.setNetworkMode(value(environment, "QUERYWEAVER_EXECUTION_NETWORK_MODE", properties.getNetworkMode()));
		properties.setImageName(value(environment, "QUERYWEAVER_EXECUTION_IMAGE", properties.getImageName()));
		properties
			.setCodeTimeout(value(environment, "QUERYWEAVER_EXECUTION_CODE_TIMEOUT", properties.getCodeTimeout()));
		properties.setReadOnlyRootFilesystem(booleanValue(environment, "QUERYWEAVER_EXECUTION_READ_ONLY_ROOT",
