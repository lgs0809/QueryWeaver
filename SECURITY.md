...
SemEvoSQL is designed to connect to business databases with read-only credentials. Production deployments should enable JWT authentication, keep application and worker control-plane endpoints on private networks, terminate TLS at a trusted reverse proxy or ingress, and treat Docker daemon access as a privileged host capability.
